...
To build a mapping you must go through five stages, indicated by different tabs in the interface.
...
4. Normalization can be done when the mapping is complete, and you can verify that the mapping is correct by stepping through records and seeing the results in a kind of dry-run.

5. When the normalized output file shows records the way they should be structured and you are satisfied, the final tab allows you to fill in some details and upload data to the metadata repository.
...
## Step One : Analysis, Record Root Choice, Unique Element Choice, Constants
...
Beside the root selection button in the interface is another button called **Select Unique Element** which works in a similar way.  You select an element from the tree, which is to be field with values that are different for every record in the data set, and then you press this button.  This is used in the metadata repository to identify records and build persistent URIs.
...
## Step Five : Uploading to Repository
