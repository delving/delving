...
<#assign seq = result.briefDocWindow.docs/>
...
<div id="navResultTabs">
    <ul>
        <li class="${showAll}">
            <a href="${thisPage}?query=${query}&amp;view=${view}"><em><@spring.message 'All_t' /> </em></a></li>
        <li class="${showText}">
            <a href="${thisPage}?query=${query}${textUrl!"&amp;qf=TYPE:TEXT"}&amp;tab=text&amp;view=${view}"><em><@spring.message 'Texts_t' />
                <@print_tab_count showAll showText textCount />
                </em></a></li>
        <li class="${showImage}">
            <a href="${thisPage}?query=${query}${imageUrl!"&amp;qf=TYPE:IMAGE"}&amp;tab=image&amp;view=${view}"><em><@spring.message 'Images_t' />
                <@print_tab_count showAll showImage imageCount />
            </em></a></li>
        <li class="${showVideo}">
            <a href="${thisPage}?query=${query}${videoUrl!"&amp;qf=TYPE:VIDEO"}&amp;tab=video&amp;view=${view}"><em><@spring.message 'Videos_t' />
                <@print_tab_count showAll showVideo videoCount />
            </em></a></li>
        <li class="${showSound}">
            <a href="${thisPage}?query=${query}${audioUrl!"&amp;qf=TYPE:SOUND"}&amp;tab=sound&amp;view=${view}"><em><@spring.message 'Sounds_t' />
    ...
    <tr>
        ...
        <td valign="bottom" width="25%" class="${cell.type}">
            <div class="brief-thumb-container">
                <a href="full-doc.html?${queryStringForPresentation}&amp;tab=${tab}&amp;start=${cell.index?c}&amp;startPage=${pagination.start?c}&amp;uri=${cell.id}&amp;view=${view}&amp;pageId=bd">
                    <#if useCache="true">
                    ...
                    <#else>
                        <img class="thumb" id="thumb_${cell.index?c}" align="middle" src="${cell.thumbnail}" alt="Click for more information" height="110" onerror="showDefault(this,'${cell.type}')"/>
                    </#if>
                </a>
            </div>
...
<table cellspacing="1" cellpadding="0" width="100%" border="0" summary="search results" id="multi" class="list">
    ...
    <tr>
            ...
            <div class="brief-thumb-container-listview">
                <a href="full-doc.html?${queryStringForPresentation}&amp;tab=${tab}&amp;start=${cell.index?c}&amp;startPage=${pagination.start?c}&amp;uri=${cell.id}&amp;view=${view}&amp;pageId=bd">
                    ...
                    <#else><img class="thumb" id="thumb_${cell.index?c}" align="middle" src="${cell.thumbnail}"
                                ...
                                onerror="showDefault(this,'${cell.type}')"/>
                    </#if>
                </a>
            </div>
        </td>
        <td class="${cell.type}">
            <div>
                <h3>
                    <a class="fg-gray" href="full-doc.html?${queryStringForPresentation}&amp;tab=${tab}&amp;start=${cell.index?c}&amp;startPage=${pagination.start?c}&amp;uri=${cell.id}&amp;view=${view}&amp;pageId=bd"><@stringLimiter "${cell.title}" "250"/></a>
                </h3>
                <#-- without labels -->
